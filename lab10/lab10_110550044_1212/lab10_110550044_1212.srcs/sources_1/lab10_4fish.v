...
reg  [31:0] fish_clock, fish_clock2, fish_clock3, fish_clock4;
wire [9:0]  pos, pos2, pos3, pos4;
...
initial begin
  ...
  fish_addr1[4] = FISH_W2*FISH_H2*4;
  fish_addr1[5] = FISH_W2*FISH_H2*5;
  fish_addr1[6] = FISH_W2*FISH_H2*6;
  fish_addr1[7] = FISH_W2*FISH_H2*7;
  ...
  fish_addr3[1] = FISH_W2*FISH_H2; /* Addr for fish image #2 */
  fish_addr3[2] = FISH_W2*FISH_H2*2;
  fish_addr3[3] = FISH_W2*FISH_H2*3;
  fish_addr3[4] = FISH_W2*FISH_H2*4;
  fish_addr3[5] = FISH_W2*FISH_H2*5;
  fish_addr3[6] = FISH_W2*FISH_H2*6;
  fish_addr3[7] = FISH_W2*FISH_H2*7;
end
...
sram3 #(.DATA_WIDTH(12), .ADDR_WIDTH(18), .RAM_SIZE(FISH_W2*FISH_H2*8))
  ram3 (.clk(clk), .we(sram_we), .en(sram_en),
          .addr(sram_addr4), .data_i(data_in), .data_o(data_out4));

sram4 #(.DATA_WIDTH(12), .ADDR_WIDTH(18), .RAM_SIZE(FISH_W2*FISH_H2*8))
...
assign pos4 = fish_clock4[31:19];            
                    
always @(posedge clk) begin
      if(~reset_n || fish_clock[31:21] > VBUF_W + FISH_W) begin
        fish_clock <= 0;
      end  else if(stop && (choose == 0 || choose == 4)) begin
...
always @(posedge clk) begin
      if (~reset_n || fish_clock2[31:20] > VBUF_W + FISH_W) begin
        fish_clock2 <= 0;
      end  else if(stop && (choose == 1 || choose == 4)) begin
...
always @(posedge clk) begin
  ...
  end  else if(stop && (choose == 2 || choose == 4)) begin
...
always @(posedge clk) begin
      if (~reset_n || fish_clock4[31:20] > VBUF_W + FISH_W) begin
        fish_clock4 <= 0;
      end  else if(stop && (choose == 3 || choose == 4)) begin
        fish_clock4 <= fish_clock4;
      end else begin
...
assign fish_region3 =
           pixel_y >= ((FISH_VPOS4)<<1) && pixel_y < (FISH_VPOS4 + FISH_H2)<<1 &&
...
always @ (posedge clk) begin
  if (~reset_n) begin
    ...
    pixel_addr5 <= fish_addr3[fish_clock[25:23]] +
                  ((pixel_y>>1)-(FISH_VPOS4))*FISH_W2 +
                  ((pixel_x +(FISH_W2*2-1)-pos4)>>1);
    pixel_addr4 <= fish_addr2[fish_clock[25:23]] +
...
always @(*) begin
  if (~video_on)
    ...
    rgb_next = data_out4;
  else if(fish_region3 && data_out5 != 12'h0f0) 
    rgb_next = data_out5;
  else
